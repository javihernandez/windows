    ...
    if (themeData && themeData.Theme) {
        ...
        if (windows1809) {
            ...
            if (highContrastId) {
                ...
                var displayNameSync;
                try {
                    var fileName = path.join(__dirname, "GetHCName.exe");
                    var displayNameSyncBuf = child_process.execFileSync(fileName, ["@%SystemRoot%\\System32\\themeui.dll,-" + options.env.stringId], options);
                    displayNameSync = displayNameSyncBuf.toString("utf8");
                    if (displayNameSync !== "undefined") {
                        displayName = options.env.stringId && displayNameSync;
                    }
                } catch (err) {
                    fluid.log("ERROR: Failed to call 'GetHCName.exe' with error '" + err + "'");
                }
            }
        }
